...
import type { ColorPreset, RGB } from '../types/index';
...
/**
 * Calculate perceived luminance (for contrast checking)
 ...
 * Returns ratio 1-21 (higher = more contrast)
 */
...
/**
 * Check if color meets WCAG AA contrast requirement (4.5:1 for text)
 */
export function meetsWCAG_AA(color1: RGB, color2: RGB): boolean {
 ...
 * t: 0-1 (0=color1, 1=color2)
 */
export function interpolateRGB(color1: RGB, color2: RGB, t: number): RGB {
  const tt = Math.max(0, Math.min(1, t));
  return [
    Math.round(color1[0] + (color2[0] - color1[0]) * tt),
    Math.round(color1[1] + (color2[1] - color1[1]) * tt),
    Math.round(color1[2] + (color2[2] - color1[2]) * tt),
  ];
}

/**
 * Interpolate between two hues (shortest path)
 * t: 0-1 (0=hue1, 1=hue2)
 */
export function interpolateHue(hue1: number, hue2: number, t: number): number {
  const h1 = clampHue(hue1);
  const h2 = clampHue(hue2);

  // Find shortest path around the wheel
  let diff = h2 - h1;
  if (diff > 180) diff -= 360;
  if (diff < -180) diff += 360;

  return clampHue(h1 + diff * t);
}
...
export interface GlowParameters {
  radius0: number;
  radius1: number;
  radius2: number;
  opacity0: number;
 ...
 * Returns blur radii and opacities for multi-layer glow effect
 */
export function getGlowParameters(brightness: number, glowIntensity: number = 1): GlowParameters {
  const b = Math.max(0, Math.min(1, brightness / 255)); // Normalize 0-1
  const g = Math.max(0, Math.min(1, glowIntensity)); // Normalize 0-1
...
/**
 * Calculate CSS box-shadow for LED glow effect
 */
export function getGlowBoxShadow(rgb: RGB, brightness: number, glowIntensity: number = 1): string {
  const glow = getGlowParameters(brightness, glowIntensity);
  const [r, g, b] = rgb;
  const color0 = `rgba(${r}, ${g}, ${b}, ${glow.opacity0})`;
  const color1 = `rgba(${r}, ${g}, ${b}, ${glow.opacity1})`;
  const color2 = `rgba(${r}, ${g}, ${b}, ${glow.opacity2})`;

  return `0 0 ${glow.radius0}px ${color0}, 0 0 ${glow.radius1}px ${color1}, 0 0 ${glow.radius2}px ${color2}`;
}

// ============ Color Bleeding Effect ============

/**
 * Calculate color contribution from adjacent pixels
 * Pixels share ~10% of their glow with neighbors
 */
export function applyColorBleeding(rgb: RGB, neighbors: RGB[], strength: number = 0.1): RGB {
  let [r, g, b] = rgb;

  for (const neighbor of neighbors) {
    r += neighbor[0] * strength;
    g += neighbor[1] * strength;
    b += neighbor[2] * strength;
  }
