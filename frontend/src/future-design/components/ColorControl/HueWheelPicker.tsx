 ...
 * - Visual indicator of current hue
 ...
 */
export const HueWheelPicker: React.FC<HueWheelPickerProps> = ({ hue, onChange, compact = false }) => {
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const normalizedHue = ((hueValue % 360) + 360) % 360;
  ...
  return (
    <div className={styles.hueWheelContainer}>
      <canvas
        ref={canvasRef}
        width={size}
        height={size}
      ...
      />
      <p className={styles.hueHint}>← Click or drag to select hue →</p>
    </div>
  );
};
