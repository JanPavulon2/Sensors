...
# from services import AnimationService, ZoneService, ApplicationStateService
...
#     ):
#         """
#         Initialize LED Controller (Layer 3 - Application Logic)

#         Args:
#             config_manager: Configuration manager (provides all config + sub-managers)
#             event_bus: Event bus for subscribing to hardware events
#             gpio_manager: GPIO manager for pin allocation tracking
#             zone_service: Zone service (injected from main)
#             animation_service: Animation service (injected from main)
#             app_state_service: Application state service (injected from main)
#             preview_panel_controller: Preview panel controller (L2, injected from main)
#             strip: Main LED strip (L1, injected from main)

#         Architecture:
#             - Receives all dependencies via Dependency Injection
#             - Does NOT create hardware (L1) or controllers (L2)
#             - Does NOT create services or DataAssembler (created in main)
#             - Focuses purely on application logic and orchestration
#         """
...
#         # Extract managers from config_manager (guaranteed non-None after load())
...
#         self.main_mode: MainMode = state.main_mode
#         self.edit_mode: bool = state.edit_mode
#         self.lamp_white_mode: bool = state.lamp_white_mode
#         self.lamp_white_saved_state: Optional[Dict] = state.lamp_white_saved_state
#         self.current_param: ParamID = state.current_param
#         self.current_zone_index: int = state.current_zone_index

#         # Prepare parameter lists
#         self.static_params = [
#             pid for pid in self.parameter_manager.get_zone_parameters().keys()
#             if pid != ParamID.ZONE_REVERSED
#         ]

#         self.animation_params = [
#             pid for pid in self.parameter_manager.get_animation_parameters().keys()
#             if pid in [ParamID.ANIM_SPEED, ParamID.ANIM_INTENSITY]
#         ]
...
#         """Initialize animation engine with injected strip and zones"""
#         all_zones = self.zone_service.get_all()
        
#         self.zone_ids = [z.config.id for z in all_zones]
        
#         self.animation_engine = AnimationEngine(self.zone_strip_controller.zone_strip, all_zones)
#         self.available_animation_ids = [
#             a.config.id for a in self.animation_service.get_all()
#         ]

#         log.info(LogCategory.SYSTEM, "Animation engine initialized")
...
#             lambda e: self.handle_selector_rotation(cast(EncoderRotateEvent, e).delta), 
#             filter_fn=lambda e: e.source == EncoderSource.SELECTOR
#         )
        
#         self.event_bus.subscribe(
#             EventType.ENCODER_CLICK, 
#             lambda e: self.handle_selector_click(), 
#             filter_fn=lambda e: e.source == EncoderSource.SELECTOR
#         )
...
#         self.event_bus.subscribe(
#             EventType.ENCODER_ROTATE, 
#             lambda e: self.handle_modulator_rotation(cast(EncoderRotateEvent, e).delta), 
#             filter_fn=lambda e: e.source == EncoderSource.MODULATOR
#         )
        
#         self.event_bus.subscribe(
#             EventType.ENCODER_CLICK, 
#             lambda e: self.handle_modulator_click(), 
#             filter_fn=lambda e: e.source == EncoderSource.MODULATOR
#         )
...
#             lambda e: self.toggle_edit_mode(), 
#             filter_fn=lambda e: e.source == ButtonID.BTN1
#         )
        
#         self.event_bus.subscribe(
#             EventType.BUTTON_PRESS, 
#             lambda e: self.quick_lamp_white(), 
#             filter_fn=lambda e: e.source == ButtonID.BTN2)
        
#         self.event_bus.subscribe(
#             EventType.BUTTON_PRESS, 
#             lambda e: self.power_toggle(), 
#             filter_fn=lambda e: e.source == ButtonID.BTN3)
        
#         self.event_bus.subscribe(
#             EventType.BUTTON_PRESS, 
#             lambda e: self.toggle_main_mode(), 
#             filter_fn=lambda e: e.source == ButtonID.BTN4)

#         log.info(LogCategory.SYSTEM, "Event handlers registered")
...
#         """Apply persisted zone colors to hardware."""
#         for zone in self.zone_service.get_all():
#             self.zone_strip_controller.render_zone(zone.config.id, zone.state.color, zone.brightness)

#     def handle_selector_rotation(self, delta: int):
#         log.debug(LogCategory.SYSTEM, f"Selector rotated {delta}")

#     def handle_selector_click(self):
#         log.debug(LogCategory.SYSTEM, "Selector clicked")

#     def handle_modulator_rotation(self, delta: int):
#         log.debug(LogCategory.SYSTEM, f"Modulator rotated {delta}")

#     def handle_modulator_click(self):
#         log.debug(LogCategory.SYSTEM, "Modulator clicked")

#     def toggle_edit_mode(self):
#         self.edit_mode = not self.edit_mode
#         log.info(LogCategory.SYSTEM, f"Edit mode: {self.edit_mode}")
...
#         )
#         log.info(LogCategory.SYSTEM, f"Toggled mode → {self.main_mode.name}")
#         self._sync_preview()
        
#     def _sync_preview(self):
#         """Sync preview according to current mode."""
#         if self.main_mode == MainMode.STATIC:
#             zone = self.zone_service.get_zone(self.zone_service.get_all()[self.current_zone_index].config.id)
#             rgb = zone.state.color.to_rgb()
#             self.preview_panel_controller.show_color(rgb, zone.brightness)
#         else:
#             current_anim = self.animation_service.get_current()
#             if current_anim:
#                 speed = current_anim.get_param_value(ParamID.ANIM_SPEED)
#                 hue = current_anim.get_param_value(ParamID.ANIM_PRIMARY_COLOR_HUE)
#                 self.preview_panel_controller.start_animation_preview(current_anim.config.id, speed=speed, hue=hue)
...
#             self.preview_panel_controller.show_bar(value, max_value, base_color)
#         else:
#             rgb = zone.state.color.to_rgb()
#             brightness = zone.brightness
...
#                 self.zone_strip_controller.set_zone_color(zone_id, r, g, b)
#                 await asyncio.sleep(cycle_duration / steps)
...
#         self.zone_strip_controller.render_zone(old_zone_id, self.zone_service.get_rgb())
...
#                 self.zone_strip_controller.power_off()
...
#                     color = self.zone_service.get_zone(zone_id).state.color
#                     target_colors[zone_id] = (color, saved)
...
#             # target_frame = self.zone_strip_controller.zone_strip.build_frame_from_zones(target_colors)

#             self.zone_strip_controller.power_on(target_colors)
