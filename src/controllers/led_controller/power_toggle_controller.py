...
class PowerToggleController:
    ...
    async def _power_off(self, zones):
        ...
        log.info(LogCategory.TRANSITION, "Power OFF - fading out")
        await self.strip_controller.transition_service.fade_out(transition)
    ...
    async def _power_on(self, zones):
        """Restore brightness and fade in"""
        from models.transition import TransitionConfig
        ...
        frame = self.strip_controller.zone_strip.build_frame_from_zones(color_map)

        log.info(LogCategory.TRANSITION, "Power ON - fading in")
        await self.strip_controller.transition_service.fade_in(frame, transition_config)
