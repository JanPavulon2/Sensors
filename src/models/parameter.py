...
# Global parameter registry (populated by load_parameters())
PARAMETERS: Dict[ParamID, Parameter] = {}


def load_parameters(config_path: str = "src/config/config.yaml") -> Dict[ParamID, Parameter]:
    """
    Load parameter definitions from config.yaml

    Loads from three sections:
    - zone_parameters: ZONE_COLOR, ZONE_BRIGHTNESS, ZONE_REVERSED
    - animation_base_parameters: ANIM_SPEED, ANIM_COLOR_1
    - additional_animation_parameters: ANIM_INTENSITY, LENGTH, HUE_OFFSET, etc.
    ...
    Raises:
        ...
        ValueError: If YAML is malformed or references unknown ParamID
    """
    # Resolve path relative to src/ directory
    # __file__ is in src/models/, so parent.parent gives us project root
    ...
    # Load from all parameter sections
    sections = [
        'zone_parameters',
    ...
    for section in sections:
        ...
        for param_name, param_data in section_data.items():
            ...
            try:
                ...
                raise ValueError(f"Unknown ParamID in {section}: {param_name}")

            # Convert string to ParameterType enum
            try:
                param_type = ParameterType[param_data['type']]
            except KeyError:
                raise ValueError(f"Unknown ParameterType for {param_name}: {param_data.get('type')}")

            # Build Parameter object
            params[param_id] = Parameter(
                id=param_id,
                type=param_type,
...
def get_parameter(param_id: ParamID) -> Parameter:
    """
    ...
    Raises:
        ...
        RuntimeError: If parameters haven't been loaded yet

    Note: Call load_parameters() during app initialization before using this function.
    """
    if not PARAMETERS:
        raise RuntimeError("Parameters not loaded. Call load_parameters() during app initialization.")
